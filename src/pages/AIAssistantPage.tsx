...
export default function AIAssistantPage() {
  ...
  useEffect(() => {
    ...
    const audioTimer = setTimeout(() => { startContinuousListening(); }, 1200);
    return () => {
  ...
  const executeVocalReply = (text: string) => {
    ...
    utterance.onerror = () => { setSpeaking(false); isUserTurnRef.current = true; startContinuousListening(); };
    ...
    try {
      ...
      if (currentGlobalState === 'IDLE') {
        ...
        if (isLookingForRemaining) {
          const msg = `العميل ${order.customer_name} متبقي عليه ${remain} جنيه من السعر الكلي البالغ ${order.price} جنيه.`;
          setAiSpeech(msg); executeVocalReply(msg); setProcessing(false); return;
        }

        if (isLookingForDate) {
          const msg = `أوردر العميل ${order.customer_name} ميعاد تسليمه المحدد هو ${order.delivery_date || 'غير محدد بعد'}.`;
          setAiSpeech(msg); executeVocalReply(msg); setProcessing(false); return;
        }
        ...
        const msg = `تم العثور على الأوردر. العميل: ${order.customer_name}، حالته الحالية: ${statusText}، والمتبقي عليه: ${remain} جنيه.`;
        setAiSpeech(msg); executeVocalReply(msg); setProcessing(false); return;
      }
  ...
  const toggleSystemMasterPower = () => {
    if (listening || speaking || processing) {
      killSpeechEngine();
      setGlobalState('IDLE');
      setAiSpeech('تم قفل المساعد الصوتي وإيقاف الميكروفون مؤقتاً.');
    } else {
    ...
    <div className="w-full h-[calc(100vh-70px)] flex flex-col justify-between items-center bg-[#020206] text-gray-100 p-5 font-sans select-none overflow-hidden relative">
      ...
      <div className="w-full max-w-5xl flex items-center justify-between border-b border-gray-900/50 pb-3.5 z-10">
        ...
        <div className="flex items-center gap-2 bg-[#080810] px-3.5 py-1.5 rounded-full border border-gray-800/60">
          ...
          <span className="text-[10px] font-black text-gray-400">{listening ? 'مستمر في الاستماع' : 'المحرك واقف'}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center my-auto z-10 w-full">
        ...
        <div className="relative cursor-pointer" onClick={toggleSystemMasterPower}>
          ...
          <button className={`w-44 h-44 rounded-full flex flex-col items-center justify-center transition-all duration-700 border-4 shadow-2xl relative z-20 ${listening ? 'bg-gradient-to-br from-emerald-600 to-teal-500 border-emerald-300' : speaking ? 'bg-gradient-to-br from-blue-600 to-indigo-500 border-blue-300' : 'bg-[#06060c] border-gray-800'}`}>
